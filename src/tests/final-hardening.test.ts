...
import { mergeContactLeadNotificationStatus } from "../lib/contact-lead.js"
import { withRecoveryWorkflowStatus } from "../lib/public-workflow-status.js"
import { createFollowUpSession, publicSession } from "../lib/follow-up-demo.js"
import { deriveSideEffectId, executeSideEffect, InMemorySideEffectStore, SideEffectPreflightError } from "../lib/side-effect-machine.js"
...
async function run() {
  ...
  await test("preflight failure remains safely retryable", async () => {
    const store = new InMemorySideEffectStore()
    const id = deriveSideEffectId("session-test", "inbound-reply-preflight", crypto.randomUUID())
    let providerCalls = 0
